...
export function attemptRender(
  ...
  try {
    if (!(root instanceof HTMLElement))
      throw typeError(root, `validation of root in attemptRender`, [
        "HTMLElement",
      ]);
  ...
  try {
    ...
    toBeRoots.forEach((toBeRoot, i) => {
      try {
        ...
        roots[`${formatRootIdf(toBeRoot, i.toString())}`] = toBeRoot;
      } catch (eR) {
...
export function renderWatcher(
  jsx: JSX.Element,
  selector: string,
  scope: Document | DocumentFragment | HTMLElement = document
): void {
  addEventListener("DOMContentLoaded", () => {
    try {
      const watcher = scope.querySelector(selector);
      if (!watcher) throw new Error(`Failed to locate the watcher instance`);
      ReactDOM.render(jsx, watcher);
    } catch (e) {
      console.error(`Error rendering ${selector}:\n${(e as Error).message}`);
    }
  });
}
