...
class Component implements Component_Interface {
	...
	public function disable_json_optionally( $path, $file ) {
		$length = strlen( 'kadencetheme.json' );
		if ( ! empty( $file ) && 'theme.json' === $file && substr( $path, -$length ) === 'kadencetheme.json' ) {
			$theme_mode = get_theme_mod( 'theme_json_mode', false );
			if ( ! $theme_mode ) {
				// Set the path to a file that doesn't exist.
				$new_path = str_replace( 'kadencetheme.json', 'kadencemissing-theme.json', $path );
				return $new_path;
			} else {
				return $path;
			}
