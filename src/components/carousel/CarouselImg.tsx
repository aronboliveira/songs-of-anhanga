...
import { nlDiv, nlImg } from "src/lib/declarations/types";
import {
  htmlElementNotFound,
  markWithCommentary,
  stringError,
...
export default function CarouselImg(props: CarouselImgProps): JSX.Element {
  ...
  const itemRef = useRef<nlDiv>(null),
    imgRef = useRef<nlImg>(null),
    [img, setImg] = useState<JSX.Element>(<></>);
  useEffect(() => {
      ...
      try {
        ...
        if (!(relCarousel instanceof HTMLElement))
          ...
          if (imgRef.current === firstImg || currNImgs === 1) {
            ...
            const firstImage =
              imgRef.current.parentElement!.querySelector("img");
            try {
              if (
              ...
              imgRef.current.style.left =
                imgRef.current.parentElement.style.left;
            } catch (e) {
              markWithCommentary(
                firstImage,
    ...
    (async () => {
      const isValid = await (() => {
        return new Promise(resolve => {
          ...
          img.src = `/${props.subDir}${props.fullName}`;
        });
      })();
      isValid
        ? setImg(
            <div className="carousel-item" ref={itemRef}>
              <img
                ...
                alt={`${props.fullName.slice(
                  0,
                  props.fullName.lastIndexOf(".")
                )}`}
                loading="lazy"
                decoding="async"
              />
