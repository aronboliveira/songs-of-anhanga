...
export default function ModalCookies({
  ...
  useEffect(() => {
    try {
      ...
      initFillAttrs(`ModalCookies`, dlgRef.current);
      initCloseableDlg(dlgRef.current, state, dispatch, "TOGGLE");
    } catch (e) {
      markWithCommentary(dlgRef.current);
      console.error(
        `Error executing useEffect for Dialog Reference in Modal for Cookies:\n${
          (e as Error).message
        }`
      );
    }
  ...
  useEffect(() => {
    !location.href.endsWith("?cookies") &&
      history.pushState({}, ``, `?cookies`);
    return () => {
      location.href.endsWith("?cookies") && history.pushState({}, ``, "/");
    };
  }, []);
  return (
    <ErrorBoundary
      FallbackComponent={() => (
        <GenericErrorComponent message="Error loading Modal Window for Cookies!" />
      )}
    >
      {state.isOpen && (
        <dialog className="modal modal-content" ref={dlgRef}>
          <section>
            <h3>MODAL DE COOKIES</h3>
            <button
              className="btn-close"
              onClick={ev => {
                try {
                  ...
                  console.log("!Cookies: State -> " + state.isOpen);
                } catch (e) {
                  console.error(
