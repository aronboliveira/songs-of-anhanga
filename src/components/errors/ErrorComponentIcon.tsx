...
export default function ErrorComponentIcon({
  fill = false,
}: {
  fill: boolean;
}) {
  ...
  useLayoutEffect(() => {
    try {
      if (!(svgRef.current instanceof HTMLElement))
        htmlElementNotFound(
          svgRef.current,
          `validation of SVG span in ${ErrorComponentIcon.prototype.constructor.name}`,
          ["HTMLElement"]
        );
      if (fill) {
      ...
      console.error(
        `Error executing useLayoutEffect for ${
          ErrorComponentIcon.prototype.constructor.name
        }:${(e as Error).message}`
      );
    }
  }, [svgRef]);
